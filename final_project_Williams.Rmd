...
# Establish categorical variable from numeric variable indfmpir (ratio to poverty)
# and label as number for data cleaning, same with ridageyr (age) variable
...
```
Combinationdoctor, medicare, medigap, medicaid, chip, statesponsored, othergovernment, havemedicare, and in age and ratio_poverty, a category was removed due to there being no/low numbers (too much missing data) based on category, so a new data set was created to account for this. 
```{r eval = TRUE, message = FALSE}
nhanes2017_a <- subset(nhanes2017_prev, age != "Children (0-17 years)") #remove category from variable
nhanes2017_a <- subset(nhanes2017_prev, ratio_poverty != "Equal to the poverty level (1)") #remove category from variable
...
Table3 <- tbl_regression(fit_glm1,
                         ...
                         bold_labels() %>%
                         italicize_levels() %>%
                         modify_caption("Table 3. Multivariate Model:
...
forestplot1 <- plot_model(fit_glm1,
               ...
               value.offset = .3,
               show.p = TRUE,
               title = "Asthma-related ER Visits")
forestplot1
```
...
Table4 <- tbl_regression(fit_glm2,
                         ...
                         bold_labels() %>%
                         italicize_levels() %>%
                         modify_caption("Table 4. Multivariate Model:
