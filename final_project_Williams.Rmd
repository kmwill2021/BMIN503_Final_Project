...
#for data cleaning, same with age variable
...
#make new category for percentages???
#nhanes2017_prev$indfmpir[P_DEMO$indfmpir < 1] <- "Below the poverty level (<1)" <- 1
```
...
table2 <- tbl_summary(nhanes2017_prev, by = everasthma, missing = "no") %>%
  ...
  add_stat_label() %>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Table 2. Decriptive Characteristics for Asthma Prevalence**") %>%
  bold_labels
```
...
```
Ratio to poverty, combinationdoctor, and the children category in age was not added due to there being no/low numbers based on category, so a new data set was created to account for this. Children was removed from age due to low numbers.
```{r eval = TRUE, message = FALSE}
...
nhanes2017_prev_sub <- nhanes2017_a %>%
  dplyr::select(everasthma, asthmaattack, gender, age, race, education,
                maritalstatus, insurancebinary, prescription_insurance,
...
```
Age, race, insurance status, and asthma-related ER visits distribution for asthma prevalence were plotted below.
```{r eval = TRUE, message = FALSE}
p1 <- ggplot2::ggplot(data = nhanes2017_prev_sub, aes(x = age)) +
                geom_bar() +
                ggtitle(paste("Asthma Patients Age Distribution")) +
                theme(text = element_text(family = "Palatino"),
                plot.title = element_text(hjust = 0, face="bold", size=20))
p2 <- ggplot2::ggplot(data = nhanes2017_prev_sub, aes(x = race)) +
                geom_bar() +
                labs(title = "Asthma Patients Race Distribution",
                x = "Race", y = "Count") +
                theme(axis.text.x = element_text(angle = 45, hjust = 1),
                plot.title = element_text(hjust = 0, face="bold", size=20)) 
p3 <- ggplot2::ggplot(data = nhanes2017_prev_sub, aes(x = insurancebinary)) +
                geom_bar() +
                labs(title = "Asthma Patients Insurance Status",
                x = "insurancebinary", y = "Count") +
                theme(axis.text.x = element_text(angle = 45, hjust = 1),
                plot.title = element_text(hjust = 0, face="bold", size=20))
p4 <- ggplot2::ggplot(data = nhanes2017_prev_sub, aes(x = erasthma)) +
                geom_bar() +
                labs(title = "Asthma-related ER Visits",
                x = "erasthma", y = "Count") +
                theme(axis.text.x = element_text(angle = 45, hjust = 1),
                plot.title = element_text(hjust = 0, face="bold", size=20))
```
...
```{r eval = TRUE, message = FALSE}
tbl_regression(fit_glm1, exponentiate = TRUE, 
  pvalue_fun = ~style_pvalue(.x, digits = 2)) %>%
  bold_p(t = 0.10) %>%
  bold_labels() %>%
  italicize_levels() %>%
  modify_caption("Table 3. Multivariate Model: Asthma-related ER Visits")
```
In the third aim, we looked at healthcare provider visits and emergency room visits in those with asthma (asthma prevalence group) from 2017-March 2020 in the US by using a multivariate linear regression model.
```{r eval = TRUE, message = FALSE}
...
tbl_regression(fit_glm2, exponentiate = TRUE, 
  pvalue_fun = ~style_pvalue(.x, digits = 2)) %>%
  bold_p(t = 0.10) %>%
  bold_labels() %>%
  italicize_levels() %>%
  modify_caption("Table 4. Multivariate Model: Healthcare Provider
  Visits and Asthma-related ER Visits")
```
### Conclusions
The point prevalence of asthma in the total cohort population from 2017-March 2020 is 4.1%. Asthma-related emergency room visits (in 12 months) is significantly associated with being Non-Hispanic Black (p-value = 0.038, 95% CI 0.12-0.91) and being a college graduate or above (p-value = 0.009, 95% CI 1.49-15.7). Healthcare provider visits and asthma-related emergency room visits (in 12 months) is significantly associated with being Non-Hispanic Black (p-value = 0.040, 95% CI 0.11-0.92), being a college graduate or above (p-value = 0.039, 95% CI 1.07-13.1), and not going to a mental health visit in 12 months (p-value = 0.006, 95% CI 1.25-3.90). There is no significant association between the number of healthcare provider visits in 12 months and asthma-related emergency room visits.
Several relevant variables were excluded in this report, as one limitation is the number of missing data, and lack of comparable groups within variables such as the number of chlidren with asthma. Aditionally, since the data was collected in a cross-sectional design, comparisons of different variables was not possible. Future studies should include longitudinal data, to properly compare risk factors given time. This report also did not account for other health conditions and/or lifestyle choices that can be contributory to asthma-related flares such as COPD and cigarette smoking, so future studies should examine this. Lastly, this report was not weighted, so further analysis should be done to make it generalizable to the US population.
